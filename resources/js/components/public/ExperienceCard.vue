...
<template>
    <Link :href="`/certifications-career/${experience.slug}`" class="flex gap-4 rounded-2xl transition-transform hover:scale-105">
        <div
            class="flex size-20 flex-shrink-0 items-center justify-center gap-2.5 rounded-lg border bg-white p-3 shadow-[0px_0.25rem_0.5rem_0px_rgba(0,0,0,0.25)] md:p-4 dark:border-gray-700 dark:bg-gray-800"
        >
            <img
                ...
                loading="eager"
            />
            <div v-else class="flex h-full w-full items-center justify-center rounded bg-gray-200 dark:bg-gray-700">
                <span class="text-center text-xs text-gray-500 dark:text-gray-400">{{ experience.organizationName }}</span>
            </div>
        </div>
